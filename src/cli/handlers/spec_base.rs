...
use crate::specs::{SpecManager, SpecMetadata, SpecPhase, SpecProgress, SpecVersion, Specification};
use chrono::Utc;
use std::env;
...
impl SpecFormatter {
    ...
    pub fn format_status(spec: &Specification, formatter: &OutputFormatter) {
        ...
        formatter.info(&format!("   Phase: {:?}", spec.metadata.progress.current_phase));
        formatter.info(&format!("   Version: {}.{}.{}", 
            spec.metadata.version.major,
            spec.metadata.version.minor,
            spec.metadata.version.patch
        ));
    ...
    pub fn format_progress(spec: &Specification, formatter: &OutputFormatter) {
        ...
        formatter.info("\n✅ Approvals:");
        Self::format_phase_status("Requirements Approved", progress.requirements_approved, formatter);
...
pub mod validation {
    ...
    pub fn spec_exists(spec_id: &str, context: &SpecContext) -> Result<()> {
        context.spec_manager.load(spec_id)
            .map(|_| ())
            .map_err(|_| VibeTicketError::Custom(
                format!("Specification '{}' not found", spec_id)
            ))
    }
    ...
    pub fn can_transition_phase(spec: &Specification, to_phase: SpecPhase) -> Result<()> {
        ...
        if valid_transition {
            ...
            Err(VibeTicketError::Custom(
                format!("Cannot transition from {:?} to {:?} phase", current, to_phase)
            ))
        }
    }
}
